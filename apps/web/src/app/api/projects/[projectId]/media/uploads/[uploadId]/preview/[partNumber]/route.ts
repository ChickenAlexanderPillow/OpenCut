...
export async function PUT(
	...
	if (!session || !session.previewObjectKey || !session.previewMultipartUploadId) {
		return NextResponse.json(
			{ error: "Preview upload session not found or expired" },
			{ status: 404 },
		);
	}
	const totalParts = session.previewTotalParts ?? 0;
	if (parsedPartNumber > totalParts) {
		return NextResponse.json(
			{ error: "Part number exceeds preview part count" },
			{ status: 400 },
		);
	}
	...
	try {
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to upload preview part",
			},
			{ status: 500 },
		);
	}
